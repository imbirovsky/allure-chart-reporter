...
This is a test automation framework that leverages the power of `Kotlin`, `Python`, `Java`, `Maven` and `GitHub Actions`
...
3. Set up the necessary secrets in the 'Secrets' section under 'Settings' in your GitHub repository. The required
   ...
   GitHub Pages is a paid feature. You don't necessarily have to use it. In any case, you will receive a summary of the
   test results along with a chart, but without the ability to open the full report (button '🔗 Link to report').
...
- `tests.AllStatusesTests.kt`: contains tests for checking all statuses (failed, passed, broken, skipped)
