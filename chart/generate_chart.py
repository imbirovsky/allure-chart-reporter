...
def create_legend_elements(data):
    return [mlines.Line2D([0], [0], color=d["color"], marker='.', linestyle='None', markeredgecolor=d["color"],
                          markersize=8, markeredgewidth=1.2) for d in data if d["value"] > 0]
...
def generate_chart(total, passed, failed, broken, skipped, sum_duration):
    ...
    legend = ax.legend(legend_elements, labels, loc="center left", bbox_to_anchor=(1, 0, 0.5, 1), handlelength=1,
                       handletextpad=0.4)
    ...
    fancy_box1 = patches.FancyBboxPatch((-1.1, -1.1), 2.2, 2.2, boxstyle="round,pad=0.02", linewidth=1,
                                        edgecolor=(169 / 255, 169 / 255, 169 / 255, 0.3), facecolor='none', zorder=-1)
    fancy_box2 = patches.FancyBboxPatch((-1.05, -1.05), 2.1, 2.1, boxstyle="round,pad=0.02", linewidth=1,
                                        edgecolor=(169 / 255, 169 / 255, 169 / 255, 0.6), facecolor='none', zorder=-1)
